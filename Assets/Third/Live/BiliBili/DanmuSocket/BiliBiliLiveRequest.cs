...
namespace Liluo.BiliBiliLive
{
    public class BiliBiliLiveRequest : IBiliBiliLiveRequest
    {
        ...
        /// <returns></returns>
        public override async Task<bool> Connect(long roomId)
        {
            if (connected)
            {
                UnityEngine.Debug.LogWarning("连接已存在");
                return true;
            }
            //token 令牌
            var token = "";
            ...
            catch (Exception e)
            {
                ...
                UnityEngine.Debug.LogWarning($"获取弹幕服务器地址时出现错误，尝试使用默认服务器... 错误信息: {e}");
            }
            // 创建 TCP对象
        ...
        async Task ReceiveMessageLoop()
        {
            ...
            while (this.connected)
            {
                try
                {
                    ...
                    {
                        UnityEngine.Debug.LogWarning("协议失败: (L:" + protocol.PacketLength + ")");
                        continue;
                    }
                    ...
                    if (protocol.Version == 2 && protocol.Action == 5)
                    {
                        ...
                        using (var deflate = new DeflateStream(ms, CompressionMode.Decompress))
                        {
                            ...
                            {
                                UnityEngine.Debug.LogWarning($"读取弹幕消息失败。错误信息: {e}");
                                OnDataError?.Invoke($"读取弹幕消息失败。错误信息: {e}");
                                break;
                            }
                        }
                    }
                ...
                catch (Exception e)
                {
                    ...
                    {
                        UnityEngine.Debug.LogWarning($"接受消息时发生错误。错误信息: {e}");
                        OnDataError?.Invoke($"接受消息时发生错误。错误信息: {e}");
                        break;
                    }
                }
            }
                                ...
                                case "DANMU_MSG":
                                    {
                                        ...
                                        danmuData.userId = obj["info"][2][0].ToString();
                                ...
                                case "SEND_GIFT":
                                    {
                                        ...
                                        giftData.userId = obj["data"]["uid"].ToString();
                                ...
                                case "GUARD_BUY":
                                    {
                                        ...
                                        guardData.userId = obj["data"]["uid"].ToString();
                                ...
                                case "SUPER_CHAT_MESSAGE":
                                    {
                                        ...
                                        superChatData.userId = obj["data"]["uid"].ToString();
        ...
        async Task HeartbeatLoop()
        {
            ...
            {
                UnityEngine.Debug.LogWarning($"与服务器连接时发生错误。错误信息: {e}");
                _disconnect();
            }
        ...
        void _disconnect()
        {
            ...
            try
            {
                OnDanmuCallBack = null;
                OnDataError = null;
                OnRoomViewer = null;
                OnGiftCallBack = null;
                OnGuardCallBack = null;
                OnSuperChatCallBack = null;
                client.Close();
                netStream.Close();
                UnityEngine.Debug.Log("B站弹幕服务断开连接");
            }
            catch (Exception e)
            {
                UnityEngine.Debug.LogWarning($"断开连接时发生错误。错误信息: {e}");
            }
            netStream = null;
        }
        ...
        async Task<bool> SendJoinChannel(long channelId, string token)
        {
