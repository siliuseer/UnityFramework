...
namespace FairyGUI
{
    ...
    public class StageCamera : MonoBehaviour
    {
        ...
        public static void CheckMainCamera()
        {
            ...
            var stagCamera = obj == null ? CreateCamera(Name, layer) : obj.GetComponent<Camera>();

            var mainCamera = Camera.main;
            HitTestContext.cachedMainCamera = mainCamera;
            if (stagCamera != null && mainCamera != null)
            {
                stagCamera.GetUniversalAdditionalCameraData().renderType = CameraRenderType.Overlay;
                mainCamera.cullingMask &= ~layer;
                var stack = mainCamera.GetUniversalAdditionalCameraData().cameraStack;
                stack.Remove(stagCamera);
                stack.Add(stagCamera);
            }
        }
